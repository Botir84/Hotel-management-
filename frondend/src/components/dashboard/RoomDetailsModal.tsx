import { X, LogOut, Loader2, Calendar, UserCheck, ReceiptText, Edit3, Save, Clock, ArrowRight, TrendingUp, Banknote, CreditCard, Sparkles } from 'lucide-react';
...
export function RoomDetailsModal({ isOpen, room, reservation, onClose, onSuccess }: RoomDetailsModalProps) {
        ...
        try {
            ...
            if (extraChargeInfo && extraChargeInfo.extraCharge > 0) {
                await api.post('/payments/', {
                    ...
                    notes: `Uzaytirish: ${extraChargeInfo.extraDays} kun`
                });
            }
            setIsEditing(false);
            onSuccess();
    ...
    const handleCheckout = async () => {
        ...
        let confirmMessage = `№${room.number} xonadan chiqishni tasdiqlaysizmi?`;
        if (lateStatus.lateHours > 0) confirmMessage = `⚠️ KECHIKISH! Jarima: $${lateStatus.penalty}. To'lov olindimi?`;
        ...
        try {
            ...
            if (lateStatus.penalty > 0) {
                await api.post('/payments/', {
                    ...
                    method: 'cash',
                    notes: `Kechikish jarimasi: ${lateStatus.lateHours}s`
                });
            }
            onSuccess();
            onClose();
        } catch (error) { alert("Xatolik!"); } finally { setLoading(false); }
    };
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md">
            <div className={`${isDark ? 'bg-[#1e293b]' : 'bg-white'} border ${isDark ? 'border-white/10' : 'border-slate-200'} w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col transition-all`}>

                {/* Header */}
                <div className={`px-8 py-7 flex justify-between items-center border-b ${isDark ? 'border-white/5' : 'border-slate-100'} ${isDark ? 'bg-gradient-to-r from-[#5D7B93]/10 to-transparent' : 'bg-gradient-to-r from-slate-50 to-transparent'}`}>
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            {lateStatus.lateHours > 0 && !isEditing && <span className="px-2 py-0.5 bg-red-500/10 text-red-500 text-[9px] font-black uppercase tracking-widest rounded-md border border-red-500/20 animate-pulse">Late Alert</span>}
                        </div>
                        <h3 className={`text-3xl font-black tracking-tight ${isDark ? 'text-white' : 'text-slate-800'}`}>Room №{room.number}</h3>
                    </div>
                    ...
                    {!isEditing ? (
                        <>
                            ...
                            <div className={`p-5 rounded-2xl border flex items-center gap-4 ${isDark ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'}`}>
                                ...
                                <div>
                                    <p className={labelClass}>Current Guest</p>
                            ...
                            <div className={`p-6 rounded-[2rem] border space-y-4 ${isDark ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'}`}>
                                <div className="flex items-center justify-between border-b border-white/5 pb-4">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-lg font-black text-[#5D7B93]">${totalPaid.toLocaleString()}</p>
                                    </div>
                                </div>
                                    ...
                                    {reservation?.payments?.length ? reservation.payments.map((p, i) => (
                                        <div key={i} className={`flex justify-between items-center p-3 rounded-xl ${isDark ? 'bg-black/20' : 'bg-white border border-slate-100'}`}>
                                            <div className="flex flex-col">
                                                <span className={`font-black uppercase text-[9px] ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{p.method} Payment</span>
                                                <span className="text-slate-500 text-[10px]">
                                                    {new Date(p.created_at).toLocaleString('uz-UZ', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: 'short' })}
                                                </span>
                                            </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className={`p-6 rounded-[2.5rem] border ${isDark ? 'bg-[#5D7B93]/5 border-[#5D7B93]/20' : 'bg-slate-50 border-slate-200'} space-y-4`}>
                                <label className={labelClass}>Extend Stay (Until 12:00)</label>
                                    ...
                                    <div className="space-y-4 pt-2 animate-in fade-in slide-in-from-top-2">
                                        <div className={`flex items-center justify-between rounded-2xl px-6 py-5 border ${isDark ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-50 border-emerald-200'}`}>
                                            <div className="flex items-center gap-3 text-emerald-600">
                                                <TrendingUp size={22} />
                                                <div>
                                                    <p className="text-xs font-black uppercase">Extra Stay</p>
                                                    <p className="text-sm font-bold">+{extraChargeInfo.extraDays} Nights</p>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-[10px] font-bold text-emerald-600 uppercase">Amount Due</p>
                            ...
                            <div className={`p-6 rounded-[2rem] border ${isDark ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'}`}>
                                <label className={labelClass}>Change Room (Relocation)</label>
                                <select className={inputClass} value={editForm.room_id} onChange={e => setEditForm(prev => ({ ...prev, room_id: e.target.value }))}>
                                    <option value={room.id}>Stay in current №{room.number}</option>
                        ...
                        {!isEditing ? (
                            <>
                                ...
                                >
                                    ...
                                    {lateStatus.lateHours > 0 ? `Checkout + $${lateStatus.penalty}` : 'Checkout Now'}
                                </button>
                            </>
                        ) : (
                            <>
